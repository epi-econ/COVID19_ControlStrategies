...
if(length(scriptargs)==0) {
	...
	daily_work_contacts = 7.513051		# avg daily contacts at labor activities
	daily_other_contacts = 3.548544			# avg daily unavoidable contacts
...
if(length(scriptargs)>0) {
	...
	if(length(scriptargs)<=21) {
		cheby = 0
	}
	if(length(scriptargs)>21) {
		cheby = ifelse(as.character(scriptargs[22])=="cheby", 1, 0 )
	}
	grid_pieces = build_grid(S_gridlength, I_gridlength, R_gridlength, cheby=cheby)
	...
	scenario_label = as.character(scriptargs[14])	# scenario label
	...
	if(length(scriptargs)>19) {
		VSL_target_parameter = as.numeric(scriptargs[20])
	}
	if(length(scriptargs)<=19) {
		VSL = 1e07			# Value of a statistical life. Set to $10m.
	}
	if(length(scriptargs)>20) {
		target_VSL_or_Omega = as.character(scriptargs[21])
	}
	if(length(scriptargs)<=20) {
...
if(target_VSL_or_Omega=="VSL") { VSL = VSL_target_parameter }
if(target_VSL_or_Omega=="Omega") { VSL = 1e07 }
...
if(target_VSL_or_Omega=="Omega") { U_D = VSL_target_parameter }
