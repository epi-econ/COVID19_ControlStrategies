...
setwd("../../Results/Figures_SI_GDPpc_sensitivity/data/")
...
for(name in seq_along(ts_filenames)) {
      ...
      GDPpc = str_extract(label, "(?<=GDPpc_)\\d+")
      label = gsub("_GDPpc_", "", label)
      ...
      time_series_list[[name]] = cbind(read_csv(paste0(ts_filenames[name])),
                              type=paste0(label), GDPpc=as.numeric(GDPpc))
}
...
write.csv(scenarios_summary, "varyGDPpc.csv")

scenarios_summary_wide =  pivot_wider(scenarios_summary, id_cols=c("type","GDPpc"), names_from=type, values_from=c(PV_losses, cases_per_100k), names_sep="_") %>%
...
PV_losses_fudge = scenarios_summary_wide %>% filter(GDPpc==58000) %>% summarise(factor = 0.91/economic_loss_ratio) %>% pull(factor)
...
write_csv(scenarios_summary_wide_adjusted, path="../../Figures_data_FINAL/figure_SI_GDPpc_data.csv")

base <- ggplot(data=scenarios_summary_wide_adjusted)

loss_ratio <- base + 
			geom_col(aes(x=as.factor(GDPpc),y=economic_loss_ratio_ADJUSTED)) + 
			geom_hline(yintercept = 0.91, linetype="dashed") +
            theme_classic() +
            labs(y = "Percentage of decentralized loss averted by targeting", x = "GDP per capita", title = "Individual recessionary loss averted")

cases_ratio <- base + 
			geom_col(aes(x=as.factor(GDPpc),y=cases_per_100k_ratio)) + 
                  geom_hline(yintercept = 1, linetype="dashed") +
            theme_classic() +
            labs(y = "Cases/100k averted (ratio)", x = "GDP per capita", title = "Cases/100k averted by targeted isolation relative to voluntary isolation", fill = "Excess I-type\nhours supplied")

output_plot <- loss_ratio | cases_ratio
output_plot

png(file="../FigSI_GDPpc_sketch.png", width = 1000, height = 500)
output_plot
dev.off()
