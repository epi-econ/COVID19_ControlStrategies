...
setwd("../../Results/Figure_SI_RA_VSL_sensitivity/Omega_const/")
...
PV_losses_fudge = 1 #scenarios_summary_wide %>% filter(RA==0.1) %>% summarise(factor = 0.91/economic_loss_ratio) %>% pull(factor)
...
write_csv(scenarios_summary_wide_adjusted, path="../../Figures_data_FINAL/figure_SI_RA_Omega_const_data.csv")

base <- ggplot(data=scenarios_summary_wide_adjusted)

loss_ratio <- base + 
			geom_col(aes(x=as.factor(RA),y=economic_loss_ratio_ADJUSTED)) + 
			geom_hline(yintercept = scenarios_summary_wide$economic_loss_ratio[1], linetype="dashed") +
            theme_classic() +
...
png(file="../FigSI_RAsensitivity_Omega_const_sketch.png", width = 1000, height = 500)
output_plot
dev.off()
